...
MFCCS=log2(MFCCS);
MFCCS=dct(MFCCS);
%mmax=max(max(MFCCS)); mmin=min(min(MFCCS));
%MFCCS=(MFCCS-mmin)/(mmax-mmin);
MFCCS=flip(MFCCS);
%MFCCS=MFCCS/max(max(abs(MFCCS))); %Normalize for image
%MFCCS=im2uint8(MFCCS); % scale to 8-bit image
% this actually the Mel Frequency spectrum to avoid the zero element
% issues.
